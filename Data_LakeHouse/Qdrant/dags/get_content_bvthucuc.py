...
def crawl_qa_from_links():
    ...
    for idx, url in enumerate(urls, 1):
        try:
            ...
            question_tags = soup.find_all('h2', class_='entry-cau-hoi__content')
            answer_tags = soup.find_all('div', class_='entry-cau-hoi__content entry-content')

            question = question_tags[0].get_text(strip=True) if question_tags else ''
            answer = answer_tags[0].get_text(separator=' ', strip=True) if answer_tags else ''
